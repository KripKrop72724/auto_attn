...
export function AttendanceRecovery({ revision, toast }: { revision: number; toast: Toast }) {
  ...
  return (
    <div className="reconciliation-recovery">
      ...
      <section className="panel recovery-identity-panel"><div className="panel-header"><div><p className="eyebrow">IDENTITY EVIDENCE LANE</p><h2>Identity-held events stay protected</h2><p>These rows cannot be bulk delivered. Resolve exact terminal identity evidence in Users, then the safe recovery lane will pick them up automatically.</p></div><StatusBadge state="READ ONLY" /></div><div className="info-copy pattern-waiting"><Icon name="shield" /><span>{(delivery?.identity_held ?? 0).toLocaleString()} identity-held event(s) are visible in the Attendance and Users historical-backlog views.</span></div><div className="dialog-actions"><a className="button secondary" href="/attendance?view=needs-review"><Icon name="users" /> Open identity evidence</a></div></section>
