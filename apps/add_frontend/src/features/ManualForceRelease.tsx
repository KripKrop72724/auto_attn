...
export function ManualForceRelease({ devices, toast, initialConnectorId }: { devices: Device[]; toast: Toast; initialConnectorId?: string | null }) {
  ...
  return <section className="panel safe-repair manual-force" aria-label="Force release attendance">
    <header className="safe-repair-header"><div><p className="eyebrow">ADMINISTRATOR CONTROL</p><h2>Force release attendance</h2><p>Sync current terminal users, review matching punches, then approve their release.</p></div><span className="badge">Manual approval only</span></header>
    <div className="safe-repair-body">
      ...
      {scope === 'SELECTED' ? <><label>Find a device<input value={deviceSearch} onChange={e => setDeviceSearch(e.target.value)} placeholder="Name or terminal serial" /></label><div className="force-device-picker" role="group" aria-label="Select devices">{devices.filter(d => !d.is_spare && `${d.display_name} ${d.zkt?.serial}`.toLowerCase().includes(deviceSearch.toLowerCase())).map(d => <label key={d.connector_id}><input type="checkbox" checked={selected.includes(d.connector_id)} onChange={e => setSelected(old => e.target.checked ? [...old, d.connector_id] : old.filter(id => id !== d.connector_id))} /><span><strong>{d.display_name}</strong><small>{d.zkt?.serial || 'Serial not reported'} · {d.hardware_id} · {d.connected ? 'Online' : 'Unavailable'}</small></span></label>)}</div><p>{selected.length} devices selected</p></> : <p>Checks all active devices across Pakistan. Spares are excluded. Unavailable devices will be listed separately.</p>}
    ...
    {confirm && run && <Dialog titleId="force-approve-title" title={confirm === 'START' ? `Force release ${run.counts.ready.toLocaleString()} punches` : `${confirm[0]}${confirm.slice(1).toLowerCase()} this run`} description={confirm === 'START' ? `${readyDevices.length} ready devices. ${run.counts.review} punches cannot be released; ${run.counts.unavailable_devices} devices are unavailable.` : confirm === 'RESUME' ? 'ADD will refresh terminals before preparing the remaining approved punches.' : 'New records will stop entering delivery. Records already queued continue to be accounted for. Attendance is never deleted.'} onClose={() => { if (!busy) { setConfirm(null); setPassword(''); setError('') } }}><form className="dialog-body" onSubmit={approve}>
      ...
      <footer className="dialog-actions"><button className="button secondary" type="button" disabled={busy} onClick={() => { setConfirm(null); setPassword(''); setError('') }}>Cancel</button><button className="button primary" disabled={busy || !password || (confirm === 'START' && reason.trim().length < 3)}>{busy ? 'Saving…' : confirm === 'START' ? `Approve ${run.counts.ready.toLocaleString()} punches` : 'Confirm'}</button></footer></form></Dialog>}
  </section>
}

export function ForcedPill({ evidence }: { evidence: NonNullable<AttendanceEvent['force_release']> }) {
  const [open, setOpen] = useState(false)
  return <><button type="button" className="force-pill" onClick={() => setOpen(true)} aria-label="Forced attendance — view administrator decision">Forced</button>{open && <Dialog titleId="forced-evidence-title" title="Manual force release" onClose={() => setOpen(false)}><div className="dialog-body"><p>Approved by {evidence.administrator} · {dateTime(evidence.approved_at)}</p><p>{evidence.reason}</p><p>Terminal sync command {evidence.sync_command_id} · Saved user list {evidence.snapshot_id} · Audit {evidence.audit_id}</p><a href={`/attendance?view=needs-review&force_run=${evidence.run_id}`}>Open saved run and terminal-sync evidence</a><p>This label records the administrator’s decision. Oracle delivery status is shown separately.</p></div></Dialog>}</>
}
