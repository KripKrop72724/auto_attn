...
The directory disables inherited ACLs and grants full control only to the runner identity, `SYSTEM`, and local administrators. Do not grant the ADD container identity, database identity, web administrator, or ordinary deployment operator access.\n\nPlaintext PEM generation uses a unique runner `%TEMP%` directory, never the hardened vault. The Windows service runner's filesystem is never bind-mounted into Docker; short-lived named containers receive files through `docker cp` and are forcibly removed after public-key extraction or signing. DPAPI ciphertext, entropy, and public files are copied into newly created vault files so they inherit the destination ACL; `vault-manifest.json` is written last as the completion marker. A missing manifest means the creation transaction did not complete and may be retried, but existing key material must never be manually promoted or regenerated.\n\n## Fast, safe change discipline\n\nDo not use production CI/CD as an exploratory debugger. Before submitting signing-path changes, normalize a downloaded unsigned artifact into the documented flat package, use disposable RSA-3072 keys to execute all three bootloader signatures and the application signature, and inspect every signature with `espsecure signature-info-v2`. Audit Windows path, Docker mount, DPAPI, and ACL behavior as one batch. Run the full GitHub quality gate once on that audited batch; never weaken or skip the final gate to save time.
