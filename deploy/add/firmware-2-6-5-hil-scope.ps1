function Assert-Zkt265HilScope {
    param([string]$HilTargetsJson)

    if ([string]::IsNullOrWhiteSpace($HilTargetsJson)) {
        throw 'ZKT 2.6.5 signing and publication require the ordered exact HIL scope'
    }
    $expectedPath = Join-Path $PSScriptRoot 'hil-targets-2.6.5.json'
    ...
    try {
        ...
        throw 'ZKT 2.6.5 HIL scope is not valid JSON'
    }
    if ($expected.Count -ne 5 -or $actual.Count -ne $expected.Count) {
        throw 'ZKT 2.6.5 requires all five ordered HIL targets'
    }
    for ($index = 0; $index -lt $expected.Count; $index++) {
        ...
        if ($properties.Count -ne 3 -or
            ...
            throw 'ZKT 2.6.5 HIL scope contains unexpected target fields'
        }
        foreach ($field in @('connector_id', 'mac', 'terminal_serial')) {
            if ($actual[$index].$field -isnot [string] -or
                ...
                throw 'ZKT 2.6.5 HIL scope differs from the reviewed target order or identity'
            }
        }
    }
}
