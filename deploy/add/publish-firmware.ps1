...
if ($manifest.firmware_family -eq 'zkt' -and $Version -eq '2.6.3') {
    if ($PublicationMode -ne 'HIL_ONLY' -or -not [string]::IsNullOrWhiteSpace($HilTargetMac)) {
        throw 'ZKT 2.6.3 must first be published only to its exact ordered HIL scope'
    }
    . (Join-Path $PSScriptRoot 'firmware-2-6-3-hil-scope.ps1')
    Assert-Zkt263HilScope -HilTargetsJson $HilTargetsJson
}
if ($manifest.firmware_family -eq 'zkt' -and $Version -eq '2.6.4') {
    if ($PublicationMode -ne 'HIL_ONLY' -or -not [string]::IsNullOrWhiteSpace($HilTargetMac)) {
        throw 'ZKT 2.6.4 must first be published only to its exact ordered HIL scope'
    }
    . (Join-Path $PSScriptRoot 'firmware-2-6-4-hil-scope.ps1')
    Assert-Zkt264HilScope -HilTargetsJson $HilTargetsJson
}
if ($manifest.firmware_family -eq 'zkt' -and $Version -eq '2.6.5') {
    if ($PublicationMode -ne 'HIL_ONLY' -or -not [string]::IsNullOrWhiteSpace($HilTargetMac)) {
        throw 'ZKT 2.6.5 must first be published only to its exact ordered HIL scope'
    }
    . (Join-Path $PSScriptRoot 'firmware-2-6-5-hil-scope.ps1')
    Assert-Zkt265HilScope -HilTargetsJson $HilTargetsJson
}
if ($manifest.firmware_family -eq 'zkt' -and $Version -eq '2.6.6') {
    if ($PublicationMode -ne 'HIL_ONLY' -or -not [string]::IsNullOrWhiteSpace($HilTargetMac)) {
        throw 'ZKT 2.6.6 must first be published only to its exact ordered HIL scope'
    }
    . (Join-Path $PSScriptRoot 'firmware-2-6-6-hil-scope.ps1')
    Assert-Zkt266HilScope -HilTargetsJson $HilTargetsJson
}
if ($manifest.firmware_family -eq 'zkt' -and $Version -eq '2.6.7') {
    if ($PublicationMode -ne 'HIL_ONLY' -or -not [string]::IsNullOrWhiteSpace($HilTargetMac)) {
        throw 'ZKT 2.6.7 must first be published only to its exact ordered HIL scope'
    }
    . (Join-Path $PSScriptRoot 'firmware-2-6-7-hil-scope.ps1')
    Assert-Zkt267HilScope -HilTargetsJson $HilTargetsJson
}
if ($manifest.firmware_family -eq 'zkt' -and $Version -eq '2.6.8') {
    if ($PublicationMode -ne 'HIL_ONLY' -or -not [string]::IsNullOrWhiteSpace($HilTargetMac)) {
        throw 'ZKT 2.6.8 must first be published only to its exact ordered HIL scope'
    }
    . (Join-Path $PSScriptRoot 'firmware-2-6-8-hil-scope.ps1')
    Assert-Zkt268HilScope -HilTargetsJson $HilTargetsJson
}
if ($manifest.firmware_family -eq 'zkt' -and $Version -eq '2.6.9') {
    if ($PublicationMode -ne 'HIL_ONLY' -or -not [string]::IsNullOrWhiteSpace($HilTargetMac)) {
        throw 'ZKT 2.6.9 must first be published only to its exact ordered HIL scope'
    }
    . (Join-Path $PSScriptRoot 'firmware-2-6-9-hil-scope.ps1')
    Assert-Zkt269HilScope -HilTargetsJson $HilTargetsJson
}
