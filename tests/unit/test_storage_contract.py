...
@pytest.mark.parametrize("predecessor_version", ["2.6.6", "2.6.7"])
def test_268_accepts_only_signed_hil_predecessors(predecessor_version):
    engine = create_engine("sqlite+pysqlite:///:memory:")
    Base.metadata.create_all(engine)
    with Session(engine) as session:
        digest = DIAGNOSTIC_BASELINE_IMAGES[predecessor_version]
        predecessor = FirmwareRelease(
        ...
        release = FirmwareRelease(
            release_id="zone-lite-2.6.8", version="2.6.8", git_sha="a" * 40,
            image_sha256="c" * 64, image_size=1024, signing_key_id="key",
            partition_layout="zone-lite-ota-v1", minimum_bootstrap_version="2.4.12",
